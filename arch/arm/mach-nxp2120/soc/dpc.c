...
#ifndef CFG_DISP_PRI_OUT_SWAPRB
...
static void dpc_set_dev_enable(int module)
{
    ...
    uint  XResol = CFG_DISP_PRI_RESOL_WIDTH;
    uint  YResol = CFG_DISP_PRI_RESOL_HEIGHT;
	uint  bgcolor  = dpc->bgcolor;
	...
	bool  bESync = CFG_DISP_PRI_OUT_FORMAT == DPC_FORMAT_CCIR656 ? CTRUE : CFALSE;

	if (CTRUE == CFG_DISP_PRI_OUT_INTERLACE)
		YResol /= 2;
	...
	/* Set DPC */
   	if (((uint)NX_DPC_FORMAT_RGB555   == DPCFmt) ||
		...
		RDither = GDither = BDither = NX_DPC_DITHER_5BIT;
			   ...
			   ((uint)NX_DPC_FORMAT_RGB666B == DPCFmt) ||
...
#else
	if (dpc->dpcops && dpc->dpcops->enable)
		dpc->dpcops->enable(module);
#endif
}
...
static void dpc_set_dev_disable(int module)
{
	...
	{
	struct dpc_info *dpc = DPC_INFO(module);
	if (dpc->dpcops && dpc->dpcops->disable)
		dpc->dpcops->disable(module);
	}
#endif
}
...
inline static int dpc_wait_vsync(int module)
{
	...
	if (NX_DPC_GetDPCEnable(module)) {
		...
		sched_setscheduler(tsk, SCHED_FIFO, &param);	// change schedule
		#endif
		...
		sched_setscheduler(tsk, policy, &param);	// recover schedule
		#endif
	}
...
static void dpc_set_sync_info(int dev)
{
	struct dpc_info *dpc = DPC_INFO(dev);

	unsigned int h_pixel = dpc->x_res + dpc->h_fp + dpc->h_bp + dpc->h_sw;
	unsigned int v_pixel = dpc->y_res + dpc->v_fp + dpc->v_bp + dpc->v_sw;
	unsigned int pclk;
	int fps, cycle;

	DBGOUT("%s: dpc[%d], pixelclock=%d\n", __func__, dev, dpc->pixelclock);
	DBGOUT(" h pixel=%d (x=%4d, hfp=%3d, hbp=%3d, hsw=%3d)\n",
		h_pixel, dpc->x_res, dpc->h_fp, dpc->h_bp, dpc->h_sw);
	DBGOUT(" v pixel=%d (y=%4d, vfp=%3d, vbp=%3d, vsw=%3d)\n",
		v_pixel, dpc->y_res, dpc->v_fp, dpc->v_bp, dpc->v_sw);

	pclk  = dpc->pixelclock;
	fps	  = pclk ? (pclk * 100)/(h_pixel * v_pixel) : 6000;
	fps   = fps ? fps : 6000;
	cycle = (1000 * 100 * 10)/fps;
	dpc->wait_time = msecs_to_jiffies(cycle/10) + 1;	/* +1 jiffies */
	dpc->duration  = (cycle * 100);						/* usec */

	printk(KERN_INFO"dpc[%d]: irq=%d, fps=%d.%d, duration=%d.%d ms, wait=%d jiffies\n",
		dev, dpc->irq, fps/100, fps%100, dpc->duration/1000, dpc->duration%1000, dpc->wait_time);
}
...
void soc_dpc_init(void)
{
	...
	for (i = 0; num > i; i++, dpc++) {
		...
		dpc->regs.mlc_save = (uint*)&mlc_reg_save[i];
		dpc->regs.dpc_save = (uint*)&dpc_reg_save[i];
...
void soc_dpc_set_suspend(int module)
{
	...
	volatile uint *mlc_base = dpc->regs.mlc_base;
	volatile uint *dpc_base = dpc->regs.dpc_base;

	uint mlc_size = NX_MLC_GetSizeOfRegisterSet();
	uint dpc_size = NX_DPC_GetSizeOfRegisterSet();
	...
	if (dpc->dpcops && dpc->dpcops->suspend)
		dpc->dpcops->suspend(module);
	else
		dpc_set_dev_disable(module);
...
void soc_dpc_set_resume(int module)
{
	...
	/*
	 * Display controller resume
	 */
	if (dpc->dpcops && dpc->dpcops->resume) {
		dpc->dpcops->resume(module);
	} else {
		dpc_set_irq_disable(module);	/* disable interrupt */
		...
		dpc_set_irq_enable(module);		/* enable interrupt */
	}

	if (dpc->lcdops && dpc->lcdops->resume)
		dpc->lcdops->resume(module);

	if (dpc->bluops && dpc->bluops->resume)
...
int 	soc_dpc_set_vid_configure(int module, u_int fourcc, int srcw, int srch,
								int left, int top, int right, int bottom,
								u_int lu_a, u_int lu_s,
								u_int cb_a, u_int cb_s,
